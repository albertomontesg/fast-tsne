...
#include "gradient_computation_update.h"
...
typedef void(*comp_func)(float *, float *, float *, float, int, int, float *, float *, float, float);
...
void register_functions() {
    ...
    add_function(&unfold_d_unfold_nx4_mx4_vec,(char *) "unfold_d_unfold_nx4_mx4_vec");
...
double perf_test(comp_func f, int n) {
    ...
    do {
        ...
        for (size_t i = 0; i < num_runs; i++) {
            f(Y, P, Q, sum_Q, n, D, dC, uY, momentum, eta);
        }
        end = stop_tsc(start);
...
int main(int argc, char **argv) {
    ...
    base_f(Y_c, P, Q, sum_Q, N, D, dC_c, uY_c, momentum, eta);
