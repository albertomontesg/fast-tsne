...
file_prefix = f"input_dimensions@{timestamp}@{compiler}@{flags}"
...
for dim in DIMENSIONS:
    N, tf, ff, cy = read_benchmark_exact(
        "./benchmarking/" + file_prefix + f"@dim_{dim}@float@", 
        stop=3000)
...
plot(N, perf, labels=DIMENSIONS, title=title, store=True, store_name=file_prefix)
