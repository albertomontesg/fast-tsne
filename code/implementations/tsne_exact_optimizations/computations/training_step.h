...
inline void gradient_update_normalize_fast_vec_avx(float* Y, float* P,
									...
									float momentum, float eta, int current_it,
                                    int early_it, float factor) {
	const int M = 8;
	const int K = 4;
	...
	for(int n = 0; n < N; n += 4*K) {
		...
		for (int m = 0; m < N; m += M, mD += M * D) {
            ...
            if (current_it == early_it) {
                __m256 f_p = _mm256_set1_ps(factor);

                p_00 = _mm256_mul_ps(p_00, f_p);
                p_01 = _mm256_mul_ps(p_01, f_p);
                p_02 = _mm256_mul_ps(p_02, f_p);
                p_03 = _mm256_mul_ps(p_03, f_p);

                p_10 = _mm256_mul_ps(p_10, f_p);
                p_11 = _mm256_mul_ps(p_11, f_p);
                p_12 = _mm256_mul_ps(p_12, f_p);
                p_13 = _mm256_mul_ps(p_13, f_p);

                p_20 = _mm256_mul_ps(p_20, f_p);
                p_21 = _mm256_mul_ps(p_21, f_p);
                p_22 = _mm256_mul_ps(p_22, f_p);
                p_23 = _mm256_mul_ps(p_23, f_p);

                p_30 = _mm256_mul_ps(p_30, f_p);
                p_31 = _mm256_mul_ps(p_31, f_p);
                p_32 = _mm256_mul_ps(p_32, f_p);
                p_33 = _mm256_mul_ps(p_33, f_p);

                _mm256_store_ps(P+nN+m, p_00);
    			_mm256_store_ps(P+nN+N+m, p_01);
    			_mm256_store_ps(P+nN+2*N+m, p_02);
    			_mm256_store_ps(P+nN+3*N+m, p_03);

    			_mm256_store_ps(P+nN+4*N+m, p_10);
    			_mm256_store_ps(P+nN+5*N+m, p_11);
    			_mm256_store_ps(P+nN+6*N+m, p_12);
    			_mm256_store_ps(P+nN+7*N+m, p_13);

    			_mm256_store_ps(P+nN+8*N+m, p_20);
    			_mm256_store_ps(P+nN+9*N+m, p_21);
    			_mm256_store_ps(P+nN+10*N+m, p_22);
    			_mm256_store_ps(P+nN+11*N+m, p_23);

    			_mm256_store_ps(P+nN+12*N+m, p_30);
    			_mm256_store_ps(P+nN+13*N+m, p_31);
    			_mm256_store_ps(P+nN+14*N+m, p_32);
    			_mm256_store_ps(P+nN+15*N+m, p_33);
            }
